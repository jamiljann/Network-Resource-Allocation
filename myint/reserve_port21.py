...
class FindReservePort():
    ...
    def Return_gateway(self, dslam_IP):
        ...
        if (len(DSLAM_split) == 4) and (1 <= int(DSLAM_split[0]) <= 223) and (int(DSLAM_split[0]) != 127) and (int(DSLAM_split[0]) != 169 or int(DSLAM_split[1]) != 254) and (0 <= int(DSLAM_split[1]) <= 255 and  0 <= int(DSLAM_split[2]) <= 255 and 0 <= int(DSLAM_split[3]) <= 255):
            ...
            print('========finded_records:  ', finded_records)
            for record in finded_records:   
                ...
                if int(record_forth_part) > int(DSLAM_ip4):
                    if int(record_forth_part) <= Last_oct :
                        ...
                        self.Gateway_Int =  Interface.objects.filter(IP__contains = full_IP)
            ...
            if self.Gateway:
                if self.Gateway[0]['Encapsulation']:
                    ...
                    self.Gateway_subfilter_obj = Interface.objects.filter(int_Name__contains= self.Gateway_intname, Router_Name__Name = self.Gateway_routername) 
    ...
    def Find_Vlan_inrange(self, start_vlan, end_vlan):
        ...
        if self.Gateway:
            all_reserved_obj = ReservePort.objects.all()
            print('Reserved Ports ===', all_reserved_obj)
            
            if all_reserved_obj:
                self.filtered_reserved =  ReservePort.objects.filter(Newint__contains = self.Gateway_intname, Router__contains = self.Gateway_routername, PE__contains = self.Gateway_PE)
